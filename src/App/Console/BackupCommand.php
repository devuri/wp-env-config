...
class BackupCommand extends Command
{
	...
	protected function s3_upload_backup( string $local_file, string $s3objectfilekey ): void
	{
		$uploader = new S3Uploader(
			env( 'S3_BACKUP_KEY', '' ),
			env( 'S3_BACKUP_SECRET', '' ),
			env( 'S3_BACKUP_BUCKET', 'site-backups' )
			env( 'S3_BACKUP_REGION', 'us-east-1' ), // Specify the region where your S3 bucket is located
		);
